...
namespace SIM_PART
{
	...
	void DelaunayStructure::init_particules( const std::vector<Point* > & p_particules, int p_refresh_rate )
	{
		...
		std::cout << "Number of particles: " << NB_POINT << std::endl;

		_tetgen_mesh.initialize();
		_tetgen_mesh.numberofpoints = NB_POINT;
	...
	void DelaunayStructure::update_all() 
	{
		if ( _play_mode )
		{
			...
			if ( _mode_type == 0 )
			{
				#pragma omp parallel
				{
					std::vector<int> private_traveled_points( NB_POINT, -1 );
					#pragma omp for
	...
	void DelaunayStructure::compute_attract_points()
	{
		...
		#pragma omp parallel
		{
			std::vector<int> traveled_points( NB_POINT, -1 );
